...
- **Vulkan pixel format may be incorrect for sRGB content.** The pipeline currently uses `VK_FORMAT_B8G8R8A8_UNORM` rather than `VK_FORMAT_R8G8B8A8_SRGB` (`vulkan_graphics_pipeline.cpp`). This is unlikely to affect typical UI rendering but may produce incorrect results if sRGB-correct blending is required.
