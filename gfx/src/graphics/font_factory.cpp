...
namespace graphics {
    ...
    std::shared_ptr<IFont> FontFactory::GetDefaultFont(int size) {
    ...
    std::shared_ptr<graphics::IFont> FontFactory::GetFont(char const* name, int size) {
        ...
        // font not loaded. try to load from file
        auto pathIt = m_fontPaths.find(name);
        if (pathIt != m_fontPaths.end()) {
            std::shared_ptr<graphics::IFont> font = m_context.FontFromFile(pathIt->second, size);
