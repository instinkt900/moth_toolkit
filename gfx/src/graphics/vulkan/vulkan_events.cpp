...
namespace graphics::vulkan {
    ...
    std::unique_ptr<moth_ui::Event> FromGLFW(int button, int action, int mods, IntVec2 const& pos) {
        if (action == GLFW_PRESS) {
            return std::make_unique<moth_ui::EventMouseDown>(FromGLFWButton(button), ToMothUI(pos));
        } else if (action == GLFW_RELEASE) {
            return std::make_unique<moth_ui::EventMouseUp>(FromGLFWButton(button), ToMothUI(pos));
        }
        return nullptr;
    }
}
