...
std::unique_ptr<Event> FromSDL(SDL_Event const& event) {
    ...
    case SDL_KEYUP:
    case SDL_KEYDOWN: {
        KeyAction action = event.type == SDL_KEYUP ? KeyAction::Up : KeyAction::Down;
        int mods = 0;
        if ((event.key.keysym.mod & KMOD_LSHIFT) != 0)
            mods |= KeyMod_LeftShift;
        if ((event.key.keysym.mod & KMOD_RSHIFT) != 0)
            mods |= KeyMod_RightShift;
        if ((event.key.keysym.mod & KMOD_LALT) != 0)
            mods |= KeyMod_LeftAlt;
        if ((event.key.keysym.mod & KMOD_RALT) != 0)
            mods |= KeyMod_RightAlt;
        if ((event.key.keysym.mod & KMOD_LCTRL) != 0)
            mods |= KeyMod_LeftCtrl;
        if ((event.key.keysym.mod & KMOD_RCTRL) != 0)
            mods |= KeyMod_RightCtrl;
        return std::make_unique<EventKey>(action, FromSDLKey(event.key.keysym.sym), mods);
    }
