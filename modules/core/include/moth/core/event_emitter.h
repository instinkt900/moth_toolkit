...
namespace moth::core {
    ...
    /// the listeners list.
    class LambdaWrapper : public moth::core::IEventListener {
    public:
        LambdaWrapper(std::function<bool(moth::core::Event const&)> const& lambda, uint32_t id)
            : m_lambda(lambda)
            , id(id) {
        }
    ...
    private:
        std::function<bool(moth::core::Event const&)> m_lambda;
        uint32_t id;
    };
    ...
    public:
        ...
        void AddEventListener(moth::core::IEventListener* listener) {
            m_listeners.push_back(listener);
        }

        /// @brief Unregister a previously added pointer listener.
        void RemoveEventListener(moth::core::IEventListener* listener) {
        ...
        LambdaHandle AddEventListener(std::function<bool(moth::core::Event const&)> const& lambda) {
        ...
        /// @note O(n²) in the number of listeners: each callback is preceded by a
        ///       linear liveness check because listeners may remove themselves (or
        ///       each other) during emission. This is not thread-safe — add, remove,
        ///       and emit must all happen on the same thread or be externally
        ///       synchronized. Acceptable for small listener counts (the common
        ///       case); avoid registering more than ~50.
        bool EmitEvent(moth::core::Event const& event) {
            moth::core::EventDispatch dispatch(event);
            std::vector<moth::core::IEventListener*> snapshot(m_listeners);
    ...
    protected:
        ...
        std::vector<moth::core::IEventListener*> m_listeners;
