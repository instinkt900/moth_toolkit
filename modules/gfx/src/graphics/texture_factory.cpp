...
namespace moth::gfx {
    ...
    bool TextureFactory::LoadTexturePack(std::filesystem::path const& path) {
        ...
        if (!std::filesystem::exists(path)) {
            moth::core::log::error("LoadTexturePack: '{}' does not exist", path.string());
            return false;
        }

        std::ifstream ifile(path);
        if (!ifile.is_open()) {
            moth::core::log::error("LoadTexturePack: failed to open '{}'", path.string());
            return false;
        }
        ...
        try {
            ...
            moth::core::log::error("LoadTexturePack: failed to parse '{}': {}", path.string(), e.what());
            return false;
        }

        if (!json.contains("atlases") || !json["atlases"].is_array()) {
            moth::core::log::error("LoadTexturePack: '{}' is missing or has invalid 'atlases' array", path.string());
            return false;
        }
        ...
        for (auto&& atlasJson : json["atlases"]) {
            if (!atlasJson.is_object()) {
                moth::core::log::error("LoadTexturePack: '{}': atlas entry is not an object", path.string());
                continue;
            }
            if (!atlasJson.contains("atlas") || !atlasJson["atlas"].is_string()) {
                moth::core::log::error("LoadTexturePack: '{}': atlas entry missing 'atlas' string field", path.string());
                continue;
            }
            if (!atlasJson.contains("images") || !atlasJson["images"].is_array()) {
                moth::core::log::error("LoadTexturePack: '{}': atlas entry missing 'images' array", path.string());
                continue;
            }
            ...
            if (!texture) {
                moth::core::log::error("LoadTexturePack: failed to load atlas texture '{}'", atlasAbsPath.string());
                continue;
            }
            auto sharedTexture = std::shared_ptr<ITexture>(std::move(texture));
            for (auto&& imageJson : atlasJson["images"]) {
                if (!imageJson.is_object()) {
                    moth::core::log::error("LoadTexturePack: '{}': image entry is not an object", atlasAbsPath.string());
                    continue;
                }
                if (!imageJson.contains("path") || !imageJson["path"].is_string()) {
                    moth::core::log::error("LoadTexturePack: '{}': image entry missing 'path' string field", atlasAbsPath.string());
                    continue;
                }
                if (!imageJson.contains("rect") || !imageJson["rect"].is_object()) {
                    moth::core::log::error("LoadTexturePack: '{}': image entry missing 'rect' object", atlasAbsPath.string());
                    continue;
                }
                try {
                    ...
                    moth::core::log::error("LoadTexturePack: '{}': failed to load image entry: {}", atlasAbsPath.string(), e.what());
                    continue;
                }
            }
        }
    ...
    std::shared_ptr<ITexture> TextureFactory::GetTexture(std::filesystem::path const& path) {
        ...
        if (m_fallbackDesc) {
            moth::core::log::warn("TextureFactory: '{}' not found, using fallback", path.string());
            return m_fallbackDesc->m_texture;
        }
        moth::core::log::error("TextureFactory: '{}' not found and no fallback set", path.string());
        return nullptr;
    }
